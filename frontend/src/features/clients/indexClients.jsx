...
// Adaptador: backend -> UI shape consistente
const adaptClient = (client) => {
  ...
  if (esCaja) {
    ...
    activo = rawEstado === true || String(rawEstado).toLowerCase().trim() === "activo";
  }
...
export default function IndexClients() {
  ...
  const allClients = useMemo(() => {
    ...
    const adapted = backendArray.map(adaptClient);
  ...
  const deleteClient = async (client) => {
    ...
    // ✅ Aquí ya no asumimos boolean.
    const res = await deleteClientHook(client.id);
    ...
    // ✅ Mostrar MOTIVO (no “Error” genérico)
    Swal.fire({
      icon: "warning",
  ...
  return (
    <div className="flex min-h-screen w-full overflow-x-hidden">
      ...
      <RegisterClientModal
        ...
        allClients={allClients} // ✅ para validar unicidad en tiempo real
        onSuccess={() => {
          refetch();
