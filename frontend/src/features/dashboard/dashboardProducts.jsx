...
import { useDashboardProducts } from "../../shared/components/hooks/dashboard/dahsboardProducts.hook";
...
export default function DashboardProducts() {
  ...
  // aplicar filtros sobre los datos provenientes del hook
  const filteredTop = useMemo(
    () =>
      topProducts
        ...
        .filter((p) =>
          proveedor
            ? (p.provider ?? "").toLowerCase().includes(proveedor.toLowerCase())
            : true
        )
        .filter((p) => filterBy(p.date)),
    [topProducts, categoria, proveedor, dateRange]
  );
  ...
  const filteredStock = useMemo(
    () =>
      stockRisk
        ...
        .filter((p) => filterBy(p.date)),
    [stockRisk, categoria, proveedor, dateRange]
  );
  ...
  const stockDistribucionChart = useMemo(
    () => ({
      ...
      datasets: [
        {
          data: [
            filteredStock.filter((s) => (s.stock ?? 0) > 5).length,
            filteredStock.filter(
              (s) => (s.stock ?? 0) > 0 && (s.stock ?? 0) <= 5
            ).length,
            filteredStock.filter((s) => (s.stock ?? 0) === 0).length,
          ],
  ...
  // contadores animados
  const totalCount = useCountUp(metrics?.total ?? 0);
  const agotadosCount = useCountUp(metrics?.agotados ?? 0);
  const valorCount = useCountUp(metrics?.valorInventario ?? 0);

  // estados de carga / error
  if (isLoading) {
    return (
      <div className="p-8 min-h-screen flex items-center justify-center bg-white">
        <p style={{ color: LINE_DARK }}>Cargando productos...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="p-8 min-h-screen flex items-center justify-center bg-white">
        <p style={{ color: "red" }}>
          Ocurrió un error al cargar los productos. Verifica la API de productos.
        </p>
      </div>
    );
  }
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          >
            <div className="flex flex-col lg:flex-row gap-4 lg:items-end">
              ...
              <div className="flex-1">
                ...
                >
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                    <circle cx="12" cy="8" r="3" stroke={LINE_DARK} strokeWidth="1.6" />
                  </svg>
                  <input
                    value={proveedor}
            ...
            <div className="flex items-center gap-3 mt-4">
              ...
              <div className="ml-auto flex items-center gap-2">
                <motion.div
                  animate={
                    applied ? { scale: [1, 1.06, 1], rotate: [0, -2, 0] } : { scale: 1 }
                  }
          ...
          >
            ...
            <AnimatedCard
              ...
              value={`$${valorCount.toLocaleString()}`}
              small
            />
            <AnimatedCard
              title="Rotación"
              value={metrics?.rotacionPromedio ?? "–"}
              small
            />
          </motion.div>
          ...
          >
            ...
            <div className="overflow-x-auto">
              ...
              >
                <thead>
                  <tr>
                    <th className="p-3 text-sm" style={{ color: LINE_DARK }}>
                      Producto
                    </th>
                    <th className="p-3 text-sm" style={{ color: LINE_DARK }}>
                      Stock
                ...
                <tbody>
                  <AnimatePresence>
                    {filteredStock.map((p, i) => {
                      ...
                      return (
                        <motion.tr
                          key={p.id ?? p.label + String(p.date)}
                          custom={i}
                        ...
                        >
                          ...
                          <td className="p-3">
                            <motion.span
                              ...
                              style={{
                                background: isOut
                                  ? "linear-gradient(90deg,#ffe9e9,#ffd4d4)"
                                  : "linear-gradient(90deg,#e8f9ea,#ccf6d9)",
                  ...
                  {filteredStock.length === 0 && (
                    <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      <td
                        ...
                        style={{ color: LINE_DARK }}
                      >
                        No hay productos en riesgo para los filtros
                        seleccionados.
                      </td>
                    </motion.tr>
                  )}
